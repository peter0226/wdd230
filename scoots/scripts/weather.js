...
const currentTempMax = document.querySelector('#pBanner');
const weatherlcon = document.querySelector('#weather-icon');
const weatherlcon2 = document.querySelector('#weather-icon2');
...
function displayResults(data){
    ...
    currentTempMax.innerHTML= `${data.main.temp_max}&deg;F`;
    weatherlcon2.src=`http://openweathermap.org/img/wn/${data.weather[0].icon}.png`;
}
...
function displayForecast(data) {
    ...
    // Iterar sobre los elementos en data.list
    for (let i = 0; i <= 24; i ++) {
      // Crear elementos para cada pronóstico a las 15:00
